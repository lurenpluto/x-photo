...
pub fn router(pool: SqlitePool, config: AppConfig) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/rpc/v1/sources/{source_id}/scan",
            post(handlers::trigger_source_scan),
        )
        .route(
            "/rpc/v1/sources/{source_id}/scan:fs-watch",
            post(handlers::trigger_source_scan_fs_watch),
        )
        .route("/rpc/v1/scan-jobs/{job_id}", get(handlers::get_scan_job))
        .route(
            "/rpc/v1/scan-jobs/{job_id}/cancel",
            post(handlers::cancel_scan_job),
        )
        .route(
            "/rpc/v1/scan-jobs/{job_id}/retry",
            post(handlers::retry_scan_job),
        )
        .route("/rpc/v1/task-jobs", get(handlers::list_task_jobs))
        .route(
            "/rpc/v1/task-jobs/active",
            get(handlers::list_active_task_jobs),
        )
        .route("/rpc/v1/task-jobs/health", get(handlers::get_task_health))
        .route(
            "/rpc/v1/task-jobs/overview",
            get(handlers::get_task_overview),
        )
        .route("/rpc/v1/task-jobs/{job_id}", get(handlers::get_task_job))
        .route(
            "/rpc/v1/task-jobs/{job_id}/cancel",
            post(handlers::cancel_task_job),
        )
        .route(
            "/rpc/v1/task-jobs/{job_id}/retry",
            post(handlers::retry_task_job),
        )
        .route("/rpc/v1/photos/search", post(handlers::search_photos))
        .route(
            "/rpc/v1/photos/favorites",
            get(handlers::list_favorite_photos),
        )
        .route("/rpc/v1/photos/{photo_id}", get(handlers::get_photo_detail))
        .route(
            "/rpc/v1/photos/{photo_id}/thumb",
            get(handlers::get_photo_thumbnail),
        )
        .route(
            "/rpc/v1/photos/{photo_id}/file",
            get(handlers::get_photo_file),
        )
        .route(
            "/rpc/v1/photos/{photo_id}/remark",
            patch(handlers::update_photo_remark),
        )
        .route(
            "/rpc/v1/photos/{photo_id}/favorite",
            patch(handlers::update_photo_favorite),
        )
        .route(
            "/rpc/v1/photos/batch/delete",
            post(handlers::batch_delete_photos),
        )
        .route(
            "/rpc/v1/photos/batch/add-to-album",
            post(handlers::batch_add_to_album),
        )
        .route(
            "/rpc/v1/albums",
        ...
        .route(
            "/rpc/v1/albums/{album_id}",
            get(handlers::get_album_detail).patch(handlers::update_album),
        )
        .route(
            "/rpc/v1/albums/{album_id}/cover",
            patch(handlers::set_album_cover),
        )
        .route(
            "/rpc/v1/albums/{album_id}/photos:add",
            post(handlers::album_add_photos),
        )
        .route(
            "/rpc/v1/albums/{album_id}/photos:remove",
            post(handlers::album_remove_photos),
        )
        .with_state(state)
        .layer(cors)
        .layer(trace)
